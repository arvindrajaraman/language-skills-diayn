...
import jax
from jax import random, jit, lax, tree_util
import flax
...
class Dataset(flax.core.FrozenDict):
    ...
    Example:
        dataset = Dataset({
            'observations': {
                'image': onp.random.randn(100, 28, 28, 1),
                'state': onp.random.randn(100, 4),
            },
            'actions': onp.random.randn(100, 2),
        })
    ...
    def sample(self, batch_size: int, indx=None):
        ...
        """
        if indx is None:
            indx = onp.random.randint(self.size, size=batch_size)
        return self.get_subset(indx)

    def sample_all(self):
        return self.sample(batch_size=self.size, indx=onp.arange(self.size))
...
class ReplayBuffer(Dataset):
    ...
    Example:
        example_transition = {
            'observations': {
                'image': onp.random.randn(28, 28, 1),
                'state': onp.random.randn(4),
            },
            'actions': onp.random.randn(2),
        }
    ...
    def create(cls, transition, size: int):
        def create_buffer(example):
            example = onp.array(example)
            return onp.zeros((size, *example.shape), dtype=example.dtype)
    ...
    def create_from_initial_dataset(cls, init_dataset: dict, size: int):
        def create_buffer(init_buffer):
            buffer = onp.zeros((size, *init_buffer.shape[1:]), dtype=init_buffer.dtype)
