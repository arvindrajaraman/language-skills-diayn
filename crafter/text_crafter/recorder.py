...
class VideoRecorder:
  ...
  def reset(self):
    obs, info = self._env.reset()
    obs['obs'] = cv2.resize(obs['obs'], (64, 64), interpolation=cv2.INTER_LINEAR)
    ...
    if hasattr(self._env, 'goal_str') and self._env.goal_str is not None:
  ...
  def _save(self):
    ...
    if self._num_episodes_so_far % 50 == 0:
      print('DEBUG: saving video')
      filename = str(self._directory / (self._env.episode_name + '_{}.mp4'.format(time.time())))
